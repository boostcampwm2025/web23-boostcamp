...
// Mock previous question
const PREV_QUESTION = "SmileTogether 프로젝트에서 Web Socket과 Stomp 프로토콜을 사용한 채팅 기능을 개발하셨다고 했는데, 구체적인 구현 방식과 왜 HTTP가 아닌 Websocket을 선택했는지 설명해주실 수 있나요?";

// Nonsense answers to test
...
async function fetchAIQuestion(answer: string, attempt: number) {
  // Construct user prompt matching InterviewAIService
  const userPrompt = `
            ***[컨텍스트: 이력서 및 포트폴리오]***
            ${USER_INFO}
  ...
  // History messages: System, Asst (Q1), User (A1)
  const messages = [
  ...
  try {
    ...
    if (!response.ok) {
       ...
       console.error(`[Error] Answer: "${answer}", Status: ${response.status}, Res: ${text}`);
       return null;
    }
    ...
    if (!jsonMatch) {
         console.error(`[Error] Answer: "${answer}" - Invalid JSON format in response.`);
         return null;
    }
    const parsed = JSON.parse(jsonMatch[0]);
    
    if (!parsed.question || parsed.question.trim() === "") {
        console.error(`[FAILURE] Answer: "${answer}" -> Empty Question Returned!`);
    } else {
        console.log(`[SUCCESS] Answer: "${answer}" -> Question: ${parsed.question}`);
    }

  } catch (error) {
    console.error(`[Error] Answer: "${answer}", Msg: ${error}`);
  }
}

async function runTest() {
  console.log("Starting Safety Test (Off-topic Answers)...");
  console.log(`Config: Temperature=0.7, TopP=0.8`);

  for (const answer of OFF_TOPIC_ANSWERS) {
    console.log(`\n--- Testing Answer: "${answer}" ---`);
    await fetchAIQuestion(answer, 1);
