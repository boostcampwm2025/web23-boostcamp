...
async function runSession(sessionId: number) {
  ...
  let currentAnswer = ""; // First prompt has no previous answer
  ...
  for (let turn = 1; turn <= 5; turn++) {
      ...
      console.log(`[AI Question]: ${aiResponse.question}`);
      console.log(`[Tags]: ${aiResponse.tags.join(', ')}`);
      
      // Validation Check
      if (!aiResponse.question || aiResponse.question.trim().length === 0) {
          console.error(`[CRITICAL FAILURE] Empty Question returned!`);
      }
      ...
      // Note: In real app, we don't send the user answer immediately in the SAME request,
      // but here we are preparing for the NEXT turn.
      const { answer, isNonsense } = getRandomAnswer();
      currentAnswer = answer;
      console.log(`[User Answer (${isNonsense ? 'Nonsense' : 'Valid'})]: ${currentAnswer}`);
      
      // Add User Answer to history for context
...
async function main() {
  ...
  console.log(`Starting Full Simulation: ${TOTAL_SESSIONS} sessions, 5 turns each.`);
