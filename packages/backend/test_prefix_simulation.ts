...
async function fetchAIQuestion(attempt: number) {
  // NO Response Format (Prompt only)
  const userPrompt = `
      ***[컨텍스트: 이력서 및 포트폴리오]***
      ${USER_INFO}
      ...
      ***[필수 규칙]***
      - **빈 값 금지**: "question" 및 "tags" 배열의 항목은 절대 빈 문자열("")이어서는 안 됩니다.
      - **형식 준수**: 반드시 아래 JSON 포맷으로 답변해야 합니다.
      Example:
      {
         "question": "...",
         "tags": ["..."],
    ...
    try {
      const response = await fetch(CLOVA_API_URL!, {
        ...
        body: JSON.stringify({
          ...
          topP: 0.1, // Test condition 1
          topK: 0,
          maxCompletionTokens: 1024,
          temperature: 0.4, // Test condition 1
          repeatPenalty: 1.1,
          seed: 0,
          // NO responseFormat
        }),
      ...
      if (response.status === 429) {
        console.warn(`[429 Too Many Requests] Rate limit exceeded. Waiting 60s... (Attempt ${retries + 1})`);
        await sleep(60000);
        retries++;
        continue;
      }
      ...
      const content = data.result?.message?.content;
       ...
       // Log the raw text content to see if it's JSON or text or repeatable
      console.log(`[Result #${attempt}]: ${content.substring(0, 100)}...`); 

      const jsonMatch = content.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
         try {
             const parsed = JSON.parse(jsonMatch[0]);
             console.log(` -> Parsed Q: ${parsed.question}`);
         } catch (e) {
...
async function runTest() {
  console.log("Starting TEST 1: No JSON Schema, Prompt Only, Temp 0.4, TopP 0.1");
  console.log("Iterating 100 times for the FIRST QUESTION...");
  
  for (let i = 1; i <= 100; i++) {
     process.stdout.write(`\nTest #${i}: `);
     await fetchAIQuestion(i);
     // 10s Delay
     if (i < 100) await sleep(10000);
  }
  console.log("\nTest 1 Completed.");
}
