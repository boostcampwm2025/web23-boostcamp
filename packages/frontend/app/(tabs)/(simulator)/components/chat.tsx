...
import { speakAnswer, sendAnswer, generateQuestion } from "../interview/[id]/actions";
...
export default function Chat({ 
  ...
  const appendMessage = useCallback((content: string, role: "user" | "ai", sender: string) => {
  ...
  return (
    <Card className={cn("flex h-full flex-col border-0 shadow-none", className)}>
      ...
      <div className="p-4 flex flex-col gap-2">
        {isProcessing && (
          <p className="text-xs text-blue-500 animate-pulse px-1">면접관이 생각 중입니다...</p>
        )}
        {error && (
